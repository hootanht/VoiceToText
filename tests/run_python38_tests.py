...
def run_specific_python38_tests():
    ...
    try:
        from test_python38_compatibility import TestPython38Compatibility, TestPython38SpecificFeatures
        
        suite = unittest.TestSuite()
        
        # Add Python 3.8 compatibility tests
        suite.addTest(unittest.makeSuite(TestPython38Compatibility))
        suite.addTest(unittest.makeSuite(TestPython38SpecificFeatures))
...
def generate_compatibility_report(features, packages, test_result, py38_result):
    ...
    print(f"Required Packages: {available_packages}/{total_packages} ({package_percentage:.1f}%)")
    ...
    if py38_result:
        ...
        print(f"  Passed: {py38_result.testsRun - len(py38_result.failures) - len(py38_result.errors)}")
